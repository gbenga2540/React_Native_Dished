...
import { KeyboardAvoidingView, KeyboardTypeOptions, StyleSheet, TextInput, TouchableOpacity } from 'react-native';
...
const SecureTextEntry: FunctionComponent<SecureTextEntryProps> = ({ inputValue, placeHolderText, keyboardType, setInputValue, onFocus }) => {
        ...
        return (
                <KeyboardAvoidingView style={styles.b_t_e_main}>
                        ...
                        />
                        {hidePswd ? (
                                <TouchableOpacity activeOpacity={0.65} onPress={() => setHidePswd(!hidePswd)} style={{ marginRight: 10 }}>
                                        <Feather name="eye" size={25} color={Colors.InputTextPH} />
                                </TouchableOpacity>
                        ) : (
                                <TouchableOpacity activeOpacity={0.65} onPress={() => setHidePswd(!hidePswd)} style={{ marginRight: 10 }}>
                                        <Feather name="eye-off" size={25} color={Colors.InputTextPH} />
                                </TouchableOpacity>
                        )}
